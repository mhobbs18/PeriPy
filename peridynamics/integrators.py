...
class EulerOpenCLReductionDoubleLumped(Integrator):
    r"""
    Static Euler integrator for quasi-static loading, using OpenCL kernels.
    ...
    def __init__(self, model):
        ...
        # Build the OpenCL program from file
        kernelsource = open(pathlib.Path(__file__).parent.absolute() / "kernels/opencl_euler_reduction_double_lumped.cl").read()
        SEP = " "
...
class EulerOpenCLReductionFloat(Integrator):
    ...
    def __init__(self, model):
        ...
        # Build the OpenCL program from file
        kernelsource = open(pathlib.Path(__file__).parent.absolute() / "kernels/opencl_euler_reduction_float.cl").read()
        SEP = " "
    ...
    def incrementLoad(self, model, load_scale):
        if model.num_force_bc_nodes != 0:
            ...
            self.d_force_bc_values = cl.Buffer(self.context,
                               cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR,
                               hostbuf=self.h_force_bc_values)
class EulerStochasticReduction(Integrator):
    r"""
    Stochastic Euler integrator for quasi-static loading, using OpenCL kernels.
    ...
    def __init__(self, model):
        ...
        # Build the OpenCL program from file
        kernelsource = open(pathlib.Path(__file__).parent.absolute() / "kernels/opencl_euler_stochastic_reduction.cl").read()
        SEP = " "
        ...
        # Bond forces
        self.h_forces =  np.empty((model.max_horizon_length, model.nnodes * model.degrees_freedom), dtype=np.float64)
        self.h_local_cache = np.zeros(model.max_horizon_length, dtype=np.float64)

        # Reduced to node forces
        self.h_udn = np.empty((model.nnodes, model.degrees_freedom), dtype=np.float64)
        self.h_udn1 = np.empty((model.nnodes, model.degrees_freedom), dtype=np.float64)
