...
// Macros
#define DPN 3
// MAX_HORIZON_LENGTH, PD_ETA, PD_RHO, PD_DT, PD_NODE_NO, PD_DPN_NODE_NO will be defined on JIT compiler's command line

// Calculate bond forces, reduce into nodal forces, and update acceleration, velocities and displacements
__kernel void
	TimeIntegration(
    __global double * Un,
	__global double * Udn,
	...
	)
{
	...
	if ((global_id < (PD_NODE_NO * local_size)) && (local_id >= 0) && (local_id < local_size))
    {
        ...
        if (!local_id) 
		{
			...
			// Update velocities in each direction
			Udn[DPN * node_no + 0] += Uddn[DPN * node_no + 0] * PD_DT;
			Udn[DPN * node_no + 1] += Uddn[DPN * node_no + 1] * PD_DT;
			Udn[DPN * node_no + 2] += Uddn[DPN * node_no + 2] * PD_DT;
			// Update displacements in each direction
