	...
	)
{
	...
	if (i < PD_DPN_NODE_NO)
	{
        Un[i] = ICTypes[i] == 2 ? Un[i] + (1.00 / 6.00) * PD_DT * (k1dn[i] + k2dn[i] + 4 * k3dn[i]) : Un[i] + ICValues[i];
	}
}
	...
	)
{
	...
	if (i < PD_DPN_NODE_NO)
	{
		Un2[i] = Un[i] + (PD_DT / 4) * (Udn1[i] + Udn2[i]);
	}
}
