...
def read_data(model):
    ...
    if f.mode == "r":
        ...
        for i in range(0, model.nnodes):
            iline += 1
            line = f.readline()
            damage_data[i] = np.float(line.split())
...
def main():
    ...
    for sample in range(samples):
        ...
        for realisation in range(realisations):
            ...
            damage_data, damage_sum_data, tip_displacement_data, tip_shear_force_data = model.simulate(model, realisation, sample, steps=350, integrator=integrator, write=350, toolbar=0)
            # Work out likelihood
